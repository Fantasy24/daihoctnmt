    ...
    >
      ...
      >
        ...
        <el-option
          ...
          :label="`${(item.resourceName || '').substring(0, 60)}`"
...
export default {
  props: {
    ...
    rules: {
      type: Array,
      default: () => {
        return [{ required: true, message: "Hóa chất bắt buộc nhập" }];
      },
    },
    propForm: {
      type: String,
  ...
  methods: {
    onLoadList() {
      if (!this.listData.length) {
        this.loading = true;
        apiFactory
          .callAPI(ConstantAPI["DMHC"].SEARCH_ALL)
          .then((rs) => {
            this.loading = false;
            this.lstDataTB = rs || [];
    ...
    loadFilterData(code) {
      this.loading = true;
      apiFactory
        .callAPI(ConstantAPI["DMHC"].SEARCH_ALL, {}, { id: parseInt(code) })
        .then((rs) => {
          this.loading = false;
          this.lstDataTB = rs || [];
        })
