    ...
    >
      ...
      >
        ...
        <el-option
          ...
          :key="item.toolId"
          :title="item.toolName"
          :label="`${(item.toolName || '').substring(0, 60)}`"
          :value="item.toolId"
        />
      </el-select>
    </el-form-item>
...
export default {
  props: {
    ...
    rules: {
      type: Array,
      default: () => {
        return [{ required: true, message: "Dụng cụ bắt buộc nhập" }];
      },
    },
    propForm: {
      type: String,
  ...
  methods: {
    onLoadList() {
      if (!this.listData.length) {
        this.loading = true;
        apiFactory
          .callAPI(ConstantAPI["DMDC"].SEARCH_ALL)
          .then((rs) => {
            this.loading = false;
            this.lstDataTB = rs || [];
            if (this.getList !== undefined && this.getList !== null) {
    ...
    loadFilterData(code) {
      this.loading = true;
      apiFactory
        .callAPI(ConstantAPI["DMDC"].SEARCH_ALL, {}, { id: parseInt(code) })
        .then((rs) => {
          this.loading = false;
          this.lstDataTB = rs || [];
        })
